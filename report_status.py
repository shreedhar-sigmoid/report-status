...
load_dotenv() # Loading all the envirnoment variables
...
def tracker(
    ...
    tracker_message["Subject"] = "Reports status[{yesterday_date}]".format(
    ...
    def get_reports(ORG_LIST, triggerType, triggerDay):
        ...
        for org, view in ORG_LIST:
            ...
            total_sent_reports = reportemail_coll.count(
                {
                    "emailSentTime": {"$gt": previous_day_epoch, "$lt": today_epoch},
                    "emailStatus": "c",
                    "triggerType": triggerType,
                    "orgViewReq.organization": org,
                    "orgViewReq.view": view,
            ...
            total_pending_reports = reportalert_coll.count(
                {
                    "executionOn": {"$gt": previous_day_epoch, "$lt": today_epoch},
                    "triggerStatus": "p",
                    "triggerType": triggerType,
                    "orgViewReq.organization": org,
                    "orgViewReq.view": view,
            ...
            total_inprogess_reports = reportalert_coll.count(
                {
                    "executionOn": {"$gt": previous_day_epoch, "$lt": today_epoch},
                    "triggerStatus": "i",
                    "triggerType": triggerType,
            ...
            total_No_date_reports = reportalert_coll.count(
                {
                    "executionOn": {"$gt": previous_day_epoch, "$lt": today_epoch},
                    "triggerStatus": "ND",
                    "triggerType": triggerType,
            ...
            total_failed_reports = reportalert_coll.count(
                {
                    "executionOn": {"$gt": previous_day_epoch, "$lt": today_epoch},
                    "triggerStatus": "f",
                    "triggerType": triggerType,
    ...
    def table_row(rows):
        ...
        for row in rows:
            if row[2] != sum(row[3:]):
                t += (
                    """<tr style='background-color: #FFBBCC;'>
                            <td>""" + row[0] + """</td>
                            <td>""" + row[1] + """</td>
                            <td>""" + str(row[2]) + """</td>
                            <td>""" + str(row[3]) + """</td>
    ...
    html = """\
                ...
                """.format(
        Daily_Row=table_row(daily_reports),
        yesterday_date=yesterday_date,
        Weekly_Row=table_row(weekly_reports),
    ...
    all_data_list = [
        ["daily", "everyday", daily_reports],
        ["weekly", day_of_week, weekly_reports],
        ["montly", "1st of Every Month", monthly_reports],
    ]
