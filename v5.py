            ...
            if reports_list[2] != []:
                for report in reports_list[2]:
                    ...
                    if estimated_reports_number != count_of_all_states:
                        ...
                        active_reports = report_coll.find({"reportStatus":"active","triggerType" : triggerType,"triggerDay" : triggerDay,"orgViewReq.organization" : org,"orgViewReq.view" : view},{"loginInfo.providerKey":1,"_id":1})
                        active_users = uns_coll.find({"activeStatus":True,"orgInfoList.name":org,"orgInfoList.viewInfoList.name":view},{"email":1})
                        sent_reports = reportemail_coll.find({"emailSentTime":{"$gt":start_date,"$lt":end_date},"emailStatus" : "c","triggerType":triggerType,"orgViewReq.organization":org,"orgViewReq.view":view},{"_id":1})
                        pending_reports = reportalert_coll.find({"executionOn":{"$gt":start_date,"$lt":end_date},"triggerStatus" : "p","triggerType":triggerType,"orgViewReq.organization":org, "orgViewReq.view":view},{"_id":1})
                        inprogess_reports = reportalert_coll.find({"executionOn":{"$gt":start_date,"$lt":end_date},"triggerStatus" : "i","triggerType":triggerType,"orgViewReq.organization":org,"orgViewReq.view":view},{"_id":1})
                        no_data_reports = reportalert_coll.find({"executionOn":{"$gt":start_date,"$lt":end_date},"triggerStatus" : "ND","triggerType":triggerType,"orgViewReq.organization":org,"orgViewReq.view":view},{"_id":1})
                        failed_reports = reportalert_coll.find({"executionOn":{"$gt":start_date,"$lt":end_date},"triggerStatus" : "f","triggerType":triggerType,"orgViewReq.organization":org,"orgViewReq.view":view},{"_id":1})
